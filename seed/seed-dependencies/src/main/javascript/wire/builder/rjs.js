...
/**
 * wire/builder/rjs plugin Builder plugin for requirejs (r.js) see:
 ...
 */
(function(define) {
define(['./builder/fs'], function(rhinofs) {
	...
	// replaceIdsRegex =
	// /(define)\s*\(\s*(?:\s*["']([^"']*)["']\s*,)?(?:\s*\[([^\]]+)\]\s*,)?\s*(function)?\s*(?:\(([^)]*)\))?/g;
	...
	function analyze(resourceId, require, load, config) {
		...
		function processSpec(spec) {
			...
			function scanObj(obj, path) {
				// Scan all keys. This might be the spec itself, or any
				// sub-object-literal
				// in the spec.
				for (var name in obj) {
			...
			function scanItem(it, path) {
				...
				// 1. If it's a string, and the key is module or create, then
				// assume it
	...
	function writeSpec(wireId, resourceId, write) {
		...
		if (resourceId && specIds.length === 1) {
			// Only write single modules, combined modules should come through
			// separately
